...
import { Star, CheckCircle2, Loader2, AlertCircle, UserCheck } from 'lucide-react';
...
export function TenantReviewForm({ reservationId, tenantName }: TenantReviewFormProps) {
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
                ...
                {tenantName && (
                    <p className="text-[12px] text-slate-500">
                        Comment s'est passée votre expérience avec <span className="font-bold text-slate-700">{tenantName}</span> ?
                    </p>
                )}
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows={3}
                        maxLength={500}
                        placeholder="Ponctualité, soin du véhicule, respect des règles…"
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-[13px] text-slate-800 placeholder-slate-300 outline-none focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 transition-all resize-none"
                    />
