...
import { Star, CheckCircle2, Loader2, AlertCircle, User } from 'lucide-react';
...
export function OwnerReviewForm({ reservationId, ownerName }: OwnerReviewFormProps) {
            ...
            <div className="rounded-2xl bg-white border border-slate-200/80 shadow-sm overflow-hidden">
                <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-slate-100 bg-slate-50/40">
                    ...
                    <h3 className="text-[10.5px] font-black uppercase tracking-[0.14em] text-slate-400">
                        Votre avis sur le propriétaire
                    </h3>
                </div>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
                ...
                {ownerName && (
                    <p className="text-[12px] text-slate-500">
                        Comment s'est passée votre expérience avec <span className="font-bold text-slate-700">{ownerName}</span> ?
                    </p>
                )}
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows={3}
                        maxLength={500}
                        placeholder="Communication, disponibilité, respect des engagements…"
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-[13px] text-slate-800 placeholder-slate-300 outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all resize-none"
                    />
                ...
                <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-blue-500 px-5 py-3 text-[13px] font-bold text-white shadow-sm shadow-blue-500/20 hover:bg-blue-600 hover:-translate-y-0.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:translate-y-0"
                >
                    {status === 'loading' ? (
                        <>
