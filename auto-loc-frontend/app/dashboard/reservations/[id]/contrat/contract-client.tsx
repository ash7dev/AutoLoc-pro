...
import { ArrowLeft, CheckCircle2, Clock, AlertTriangle, Info } from 'lucide-react';
import { PrintButton } from './print-button';
import { PrintRestrictionModal } from './print-restriction-modal';

/* ════════════════════════════════════════════════════════════════
   HELPERS (Duplicate or move if needed, kept here for simplicity)
   (fmtDate and fmtMoney are already in page.tsx, but used in UI)
════════════════════════════════════════════════════════════════ */
        ...
        <div className="min-h-screen bg-slate-50/80 print:bg-white">
            <div className="max-w-3xl mx-auto px-3 sm:px-4 py-6 sm:py-8 lg:px-6 lg:py-10 space-y-4 sm:space-y-6 print:py-0 print:px-0 print:space-y-0">
                ...
                <div className="rounded-2xl bg-white border border-slate-200/80 shadow-sm overflow-hidden print:border-0 print:shadow-none print:rounded-none">
                    ...
                    <div className="px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 pb-5 border-b border-slate-100">
                        ...
                        <div className="mt-3 print:hidden">
                            <PrintButton reservationId={r.id} variant="small" onClick={handlePrintClick} />
                        </div>
                    </div>
                            ...
                            {[
                                {
                                    ...
                                    items: [
                                        { bold: 'Avant confirmation (> 5 jours) :', text: 'remboursement intégral, déduction faite des frais de service AutoLoc.' },
                                        { bold: 'Avant confirmation (2-5 jours) :', text: 'remboursement à 75 % du montant total. Les 25 % restants sont retenus à titre d\'indemnisation.' },
                                        { bold: 'Après confirmation (> 3 jours) :', text: 'remboursement intégral, déduction faite des frais de service AutoLoc.' },
                                        { bold: 'Après confirmation (1-3 jours) :', text: 'remboursement à 75 % du montant total. Les 25 % restants sont retenus à titre d\'indemnisation.' },
                    ...
                    <div className="px-4 sm:px-6 lg:px-8 py-5 border-t border-slate-100">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            {/* Art. 10 — Communication et coordination */}
                            <div className="rounded-xl bg-slate-50 border border-slate-100 p-3 sm:p-4">
                                <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-wide text-slate-500 mb-2">
                                    Art. 10 — Communication et coordination
                                </p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            {[
                                ...
                                { 
                                    ...
                                    date: r.confirmeeLe ? fmtDate(r.confirmeeLe, { day: '2-digit', month: 'long', year: 'numeric' }) : fmtDate(r.creeLe, { day: '2-digit', month: 'long', year: 'numeric' })
                                },
                            ].map(({ role, name, date }) => (
                                    ...
                                    <div className="p-3.5 sm:p-4">
                                        ...
                                        <div className="flex gap-2.5 sm:gap-3">
                                            <div className="flex-1 border border-dashed border-slate-300 rounded-lg bg-slate-50 h-10 sm:h-12 flex items-center justify-center p-2">
                                                <span className="text-[10px] sm:text-[11px] font-semibold text-slate-600">{name}</span>
                                            </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 print:hidden">
                    <PrintButton reservationId={r.id} variant="large" onClick={handlePrintClick} />
                    <Link
                        href={backHref}
                ...
                {/* Modal de restriction d'impression */}
