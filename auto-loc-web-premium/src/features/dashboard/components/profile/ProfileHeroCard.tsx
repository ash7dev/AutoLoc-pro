...
import { Camera, Trash2, ShieldCheck, Star, Car, Calendar, Loader2, ArrowLeftRight, Pencil } from 'lucide-react';
...
export const ProfileHeroCard: React.FC<ProfileHeroCardProps> = ({
  ...
  const vehicleCount = profile.annoncesCount ?? profile.listingsCount ?? 0;
          ...
          <div className="flex flex-col items-center gap-6 text-center sm:flex-row sm:text-left">
              ...
              <div className="absolute bottom-0 right-0 flex items-center gap-1 rounded-full border border-white/10 bg-brand-dark p-1 shadow-lg">
                ...
                {profile.avatarUrl && (
                  <button
                    ...
                    className="rounded-full bg-rose-600/70 p-2 text-white transition-colors hover:bg-rose-600 disabled:opacity-50"
                  >
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1.5 text-[13px] text-white/60 sm:justify-start">
                <span>{profile.role === 'PROPRIETAIRE' ? 'Propriétaire hôte' : 'Locataire membre'}</span>
                {memberDate && (
                  <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-left leading-none">
                    ...
                    <span className="ml-1 text-[11px] text-white/50">
                      {vehicleCount > 1 ? 'véhicules' : 'véhicule'}
                    </span>
                  </div>
                </div>
              </div>
