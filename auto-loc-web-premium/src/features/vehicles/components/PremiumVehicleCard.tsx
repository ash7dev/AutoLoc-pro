...
export const PremiumVehicleCard: React.FC<PremiumVehicleCardProps> = ({
  ...
  return (
    ...
    <Link
      ...
      className={`group relative flex shrink-0 flex-col rounded-[28px] bg-brand-main p-2 ring-1 ring-champagne/15 shadow-[0_24px_48px_-24px_rgba(4,25,18,0.55)] transition-[transform,box-shadow] duration-500 ease-out hover:-translate-y-1 hover:shadow-[0_32px_64px_-24px_rgba(4,25,18,0.65)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-champagne motion-reduce:transition-none motion-reduce:hover:translate-y-0 ${className}`}
    >
      ...
      <div className="relative h-48 w-full overflow-hidden rounded-[22px] bg-[#062A20] sm:h-52">
        ...
        <div className="absolute bottom-3 left-4 right-3 z-10 flex items-center justify-between gap-2 text-champagne-light">
          ...
          <span className="shrink-0 rounded-full border border-champagne/30 bg-brand-main/70 px-2.5 py-0.5 text-[11px] font-medium text-champagne backdrop-blur-md">
            {vehicle.type}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col px-4 pb-4 pt-5 sm:px-5">
        ...
        <div className="mt-5 flex items-end justify-between gap-3">
          <span className="text-xs text-champagne/60">Tarif locataire par jour</span>
