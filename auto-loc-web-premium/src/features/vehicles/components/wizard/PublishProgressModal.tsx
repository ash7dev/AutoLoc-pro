...
import { Sparkles, CheckCircle2, Loader2, ShieldCheck, AlertTriangle } from 'lucide-react';
...
export const PublishProgressModal: React.FC<PublishProgressModalProps> = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#04150F]/85 p-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-[calc(1rem+env(safe-area-inset-bottom))] backdrop-blur-md">
        <motion.div
        ...
        >
          ...
          {/* Icon Header */}
          <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-2xl border border-[#4ADE80]/30 bg-brand-dark shadow-inner">
            {isError ? (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                <AlertTriangle className="h-10 w-10 text-amber-400" strokeWidth={2.5} />
              </motion.div>
            ) : isSuccess ? (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
              ...
              <Loader2 className="h-10 w-10 animate-spin text-emerald-400" strokeWidth={2.5} />
            )}
          </div>
          ...
          <div className="mb-6 space-y-2">
            <div className="relative h-3 w-full overflow-hidden rounded-full bg-white/10 p-0.5 border border-white/10">
              <motion.div
                className={`h-full rounded-full ${isError ? 'bg-amber-500' : 'bg-gradient-to-r from-[#059669] via-[#10B981] to-[#4ADE80]'}`}
                initial={{ width: '0%' }}
            ...
            <div className="flex justify-between items-center text-xs text-emerald-400/70 font-medium">
              <span>{isError ? 'Échec' : isSuccess ? 'Finalisé' : 'Progression'}</span>
          ...
          {isSuccess && (
            ...
            >
              ...
              {isEditMode ? 'Voir mes véhicules' : 'Accéder à mon annonce'}
            </motion.button>
          )}
          ...
          {isError && (
            <motion.button
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              onClick={onCloseError || onFinish}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-amber-600 hover:bg-amber-700 py-3.5 px-6 font-display font-bold text-white shadow-lg transition-transform hover:scale-[1.02] active:scale-[0.98]"
            >
