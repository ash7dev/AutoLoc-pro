        ...
        <div className="px-6 py-5 border-b border-slate-800 bg-slate-950/60 flex items-center justify-between">
          <div className="flex items-center gap-4">
            {u?.avatarUrl ? (
              ...
              <div className="w-12 h-12 rounded-full bg-emerald-950 border-2 border-emerald-500/40 text-emerald-400 font-bold text-base flex items-center justify-center">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {isStuck && (
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-violet-500/20 text-violet-300 border border-violet-500/40">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {isLoading ? (
            ...
            <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-slate-950/60 p-5 rounded-2xl border border-slate-800 space-y-4">
                    <h3 className="text-xs font-bold uppercase text-slate-400 tracking-wider flex items-center gap-2">
                      <User className="w-4 h-4 text-emerald-400" /> Informations Personnelles
                    </h3>
                    ...
                    <div className="space-y-3 text-xs">
                      ...
                      <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                        ...
                        <span className="text-white font-medium">{user?.phone || u?.telephone || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  ...
                  <div className="bg-slate-950/60 p-5 rounded-2xl border border-slate-800 space-y-4">
                    <h3 className="text-xs font-bold uppercase text-slate-400 tracking-wider flex items-center gap-2">
                      <Car className="w-4 h-4 text-emerald-400" /> Activité AutoLoc
                    </h3>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-slate-900 border border-slate-800">
                        <div className="text-slate-400 text-xs mb-1">Véhicules Possédés</div>
                        <div className="text-2xl font-black text-white font-mono">{user?._count.vehicles || 0}</div>
                      </div>
                      <div className="p-4 rounded-xl bg-slate-900 border border-slate-800">
                        <div className="text-slate-400 text-xs mb-1">Locations Réalisées</div>
                    ...
                    {isStuck && (
                      <div className="p-4 rounded-xl bg-violet-500/10 border border-violet-500/30 text-violet-300 text-xs space-y-1">
                        <div className="font-bold flex items-center gap-1.5">
                          <AlertTriangle className="w-4 h-4 text-violet-400" /> Onboarding Incomplet
                        </div>
                        <p>
              ...
              {activeTab === 'KYC' && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between bg-slate-950/60 p-4 rounded-2xl border border-slate-800">
                    <div>
                      <div className="text-sm font-bold text-white">Statut du dossier KYC: {user?.kycStatus}</div>
                      {user?.kycRejectionReason && (
                        <div className="text-xs text-rose-400 mt-0.5">Raison du rejet: {user.kycRejectionReason}</div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={isMutating}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-lg shadow-emerald-600/20 transition-all disabled:opacity-50"
                      >
                        <CheckCircle2 className="w-4 h-4" /> Valider le KYC
                      </button>
                      <button
                        onClick={() => setShowKycRejectInput(!showKycRejectInput)}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/30 text-xs font-bold transition-all"
                      >
                  ...
                  {showKycRejectInput && (
                    <div className="p-4 rounded-2xl bg-rose-950/40 border border-rose-500/40 space-y-3">
                      <label className="text-xs font-bold text-rose-200 block">Raison du rejet KYC (communiquée à l'utilisateur):</label>
                      <input
                        type="text"
                        value={kycRejectReason}
                        onChange={(e) => setKycRejectReason(e.target.value)}
                        placeholder="Ex: Document ilisible, pièce d'identité expirée..."
                        className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-rose-500/30 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                      />
                      <button
                        onClick={() => {
                        ...
                        disabled={isMutating}
                        className="px-4 py-2 rounded-xl bg-rose-600 text-white text-xs font-bold hover:bg-rose-500 transition-all"
                      >
                        Confirmer le rejet
                      </button>
                    </div>
                      ...
                      <div key={idx} className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 flex flex-col justify-between">
                        <div className="text-xs font-bold text-slate-300 mb-2 flex items-center justify-between">
                          ...
                          {doc.url && (
                            ...
                            >
                              <Eye className="w-3 h-3" /> Agrondir
                            </button>
                          )}
                        </div>
                        ...
                        {doc.url ? (
                          ...
                          <div className="h-44 rounded-xl bg-slate-900/50 border border-dashed border-slate-800 flex flex-col items-center justify-center text-slate-500 text-xs">
                            <FileText className="w-8 h-8 mb-1 text-slate-600" />
                  ...
                  {user?.vehicles && user.vehicles.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {user.vehicles.map((v) => {
                        ...
                        return (
                          <div key={v.id} className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 flex gap-4">
                            {mainPhoto ? (
                              <img src={mainPhoto} alt={v.marque} className="w-24 h-24 rounded-xl object-cover border border-slate-800" />
                            ) : (
                              <div className="w-24 h-24 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-600">
                                <Car className="w-8 h-8" />
                              </div>
                            )}

                            <div className="flex-1 min-w-0">
                              <h4 className="text-sm font-bold text-white truncate">{v.marque} {v.modele} ({v.annee})</h4>
                              <div className="text-xs text-slate-400 mt-0.5">{v.ville} • {v.prixParJour.toLocaleString()} FCFA / jour</div>
                              <div className="mt-2">
              ...
              {activeTab === 'BOOKINGS' && (
                <div className="space-y-4">
                  <h3 className="text-xs font-bold uppercase text-slate-400">Réservations Récentes</h3>
                  {user?.reservationsLocataire && user.reservationsLocataire.length > 0 ? (
                    <div className="space-y-2">
                      {user.reservationsLocataire.map((r) => (
                        <div key={r.id} className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center justify-between text-xs">
                          <div>
                            <div className="font-bold text-white">{r.vehicule}</div>
                            <div className="text-slate-400 text-[11px]">{new Date(r.creeLe).toLocaleDateString('fr-FR')}</div>
                          </div>
                          <div className="text-right">
                            <div className="font-mono font-bold text-emerald-400">{r.totalLocataire.toLocaleString()} FCFA</div>
                            <span className="text-[10px] text-slate-400 uppercase font-bold">{r.statut}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-5 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-4">
                    <h3 className="text-xs font-bold uppercase text-rose-400 tracking-wider flex items-center gap-2">
                      <ShieldAlert className="w-4 h-4 text-rose-400" /> Bannissement / Suspension du compte
                    </h3>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-medium text-slate-300 block mb-1">Durée du blocage:</label>
                        <select
                          value={banDays}
                      ...
                      <div>
                        <label className="text-xs font-medium text-slate-300 block mb-1">Raison du blocage:</label>
                        <input
                          type="text"
                          value={banReason}
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      {isBanned ? (
                        <button
                          ...
                          disabled={isMutating}
                          className="px-4 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-500 transition-all flex items-center gap-2"
                        >
                        ...
                        <button
                          ...
                          disabled={isMutating}
                          className="px-4 py-2 rounded-xl bg-rose-600 text-white text-xs font-bold hover:bg-rose-500 transition-all flex items-center gap-2"
                        >
                  ...
                  <div className="p-5 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-4">
                    <h3 className="text-xs font-bold uppercase text-amber-400 tracking-wider flex items-center gap-2">
                      <Award className="w-4 h-4 text-amber-400" /> Attribution du Rôle Utilisateur
                    </h3>
                    ...
                    <div className="flex items-center gap-4">
                      <select
                        ...
                        className="px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white font-bold focus:outline-none focus:border-amber-500"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2.5 rounded-xl bg-amber-600 text-white text-xs font-bold hover:bg-amber-500 transition-all disabled:opacity-50"
                      >
