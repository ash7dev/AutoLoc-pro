...
export interface TenantPersonalInfoCardProps {
  profile: UserProfileData;
  onEditClick: () => void;
}

export const TenantPersonalInfoCard: React.FC<TenantPersonalInfoCardProps> = ({ profile, onEditClick }) => {
  ...
  const formatBirthDate = (dateStr: string | null) => {
    if (!dateStr || !mounted) return { formatted: dateStr ? '...' : 'Non renseignée', age: null as number | null };
    try {
    ...
    <div className="rounded-3xl border border-[#041912]/8 bg-white p-6 sm:p-8 shadow-xs">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onEditClick}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border border-slate-200 px-3.5 py-2 text-[12.5px] font-semibold text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50 cursor-pointer"
        >
