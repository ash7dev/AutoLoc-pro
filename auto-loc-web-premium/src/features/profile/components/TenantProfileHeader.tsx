...
export interface TenantProfileHeaderProps {
  onRefresh?: () => void;
  isLoading?: boolean;
...
export const TenantProfileHeader: React.FC<TenantProfileHeaderProps> = ({
  onRefresh,
  isLoading = false,
  isRefreshing = false,
  lastRefreshedAt,
  ...
  const formattedRefreshedTime = (mounted && lastRefreshedAt)
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between pb-2">
      <div className="min-w-0 space-y-2">
        ...
        <p className="max-w-2xl text-xs sm:text-sm leading-relaxed text-slate-500 font-medium">
          Gérez vos informations personnelles, suivez votre statut de vérification d'identité et administrez la sécurité de votre compte AutoLoc.
        </p>
      </div>
