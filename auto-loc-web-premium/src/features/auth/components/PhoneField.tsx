      ...
      <div className="overflow-y-auto overscroll-contain">
        ...
        {(query ? filtered : [...featured, ...others]).map(c => (
          <button
            ...
            className={cn(
              'w-full flex items-center gap-3 px-3 py-2 text-left transition-colors hover:bg-slate-50 active:bg-slate-100',
        ...
        {filtered.length === 0 && (
          <p className="px-3 py-4 text-center text-[13px] text-slate-400">Aucun résultat</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="space-y-1.5">
      ...
      <div className={cn(
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2 px-3.5 py-3 border-r border-slate-100 bg-slate-50 hover:bg-slate-100 transition-colors flex-shrink-0',
            pickerOpen && 'bg-slate-100',
          )}
        >
          <span className="text-[20px] leading-none select-none">{country.flag}</span>
        ...
        <input
          ...
          className="flex-1 px-3.5 py-3 text-[14px] font-bold text-slate-800 placeholder-slate-300 outline-none bg-transparent tracking-wide min-w-0"
        />
      </div>

      {/* Dropdown pays */}
      {pickerOpen && (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          <CountryPicker
            selected={country}
