...
import { fetchApi, API_URL } from '@/lib/config';
...
export const TenantContractCard: React.FC<TenantContractCardProps> = ({
  ...
  /**
   * Règle de restriction d'impression / téléchargement complet (auto-loc-frontend) :
   * Le contrat avec numéros de téléphone est débloqué 24h avant dateDebut.
   */
  ...
  const handlePressLocked = (type: 'PAYMENT' | 'HOST' | 'COMPLETED' | 'SEALED') => {
    if (type === 'PAYMENT') {
      setInfoModalMessage({
        ...
        desc: 'Le contrat de location légal sera généré, horodaté et téléchargeable dès que le paiement de l’acompte aura été confirmé.',
      });
    } else if (type === 'HOST') {
      setInfoModalMessage({
        title: 'Contrat en cours de finalisation',
        desc: 'Votre paiement est bien confirmé ! Le contrat sera accessible dès que le propriétaire aura validé la réservation.',
      });
    } else if (type === 'COMPLETED') {
      setInfoModalMessage({
        title: 'Location terminée & archivée',
        desc: 'Cette location est officiellement terminée. Le contrat légal demeure archivé pour vos garanties et justificatifs.',
      });
    } else if (type === 'SEALED') {
      setInfoModalMessage({
        title: 'Contrat scellé & archivé',
        desc: 'Cette réservation a été annulée ou fait l’objet d’un litige. Le document reste scellé et conservé à des fins de preuve légale.',
      });
    }
  };
  ...
  const handleOpenContract = async (action: 'view' | 'download') => {
    ...
    // Vérification de la restriction d'impression/téléchargement (24h avant)
    ...
    try {
      // Endpoint NestJS : /reservations/:id/contract-access
      const data = await fetchApi<ContractAccessResponse>(`/reservations/${reservationId}/contract-access`);
      
      let rawUrl = action === 'view'
        ? (data.viewUrl || data.url || `/reservations/${reservationId}/contract/pdf`)
        : (data.downloadUrl || data.url || `/reservations/${reservationId}/contract/pdf?download=true`);
      ...
      if (action === 'download') {
        ...
        link.href = fullUrl;
        link.target = '_blank';
        link.download = `Contrat-AutoLoc-${reservationId.slice(0, 8)}.pdf`;
      ...
      console.error('Erreur accès contrat:', err);
  ...
  /* -------------------------------------------------------------------------- */
  /* CAS 1 : VERROU PAIEMENT EN ATTENTE                                         */
  /* -------------------------------------------------------------------------- */
  if (isPendingPayment) {
    return (
      <>
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-amber-800/80 font-medium mt-0.5">
                Le contrat sera généré dès le règlement de l’acompte. Cliquez pour en savoir plus.
              </p>
            </div>
          </div>
  ...
  /* -------------------------------------------------------------------------- */
  /* CAS 2 : PAIEMENT RECEVABLE EN ATTENTE CONFIRMATION HÔTE                    */
  /* -------------------------------------------------------------------------- */
  if (isPendingHost) {
    return (
      <>
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-sm font-bold text-blue-950">Contrat en préparation</h3>
              ...
              <p className="text-xs text-blue-800/80 font-medium mt-0.5">
                Le document est en cours de scellement et sera disponible dès l’accord de l’hôte.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-start gap-3.5 min-w-0">
          ...
          <div className="space-y-1 min-w-0">
            ...
            <p className="text-xs text-slate-500 font-medium leading-relaxed">
              {isSealed
                ? 'Document légalement scellé. Conservé à des fins de preuve et de juridiction. Cliquez pour plus d’informations.'
                : isCompleted
                ? 'Location achevée. Le contrat demeure accessible par accès sécurisé.'
                : restricted
                ? 'Disponible 24h avant la prise en charge pour des raisons de sécurité. Cliquez pour plus de détails.'
                : 'Document officiel horodaté avec garanties juridiques et assurances AutoLoc.'}
            </p>
          </div>
        </div>
...
/* Sous-composant Modale d'Information Générales */
function InfoModal({
  message,
  onClose,
}: {
...
/* Modale de Restriction d'Impression / Téléchargement (Identique auto-loc-frontend) */
