...
const STATUS_CONFIG: Record<string, StatusInfo> = {
  ...
  PAYEE: {
    ...
    className: 'border-[#4ADE80]/40 bg-emerald-400/15 text-emerald-400',
...
export function OwnerReservationHeroHeader({
  ...
  const days = Math.max(1, Number(nbJours || 1));
  const dailyPrice = Number((vehicule as any)?.prixParJour || 0);
  const baseCalculated = dailyPrice * days;
  const rawAmount = Number(netProprietaire || (vehicule as any)?.montantProprietaire || 0);
  ...
  return (
    <section
      ...
      className="overflow-hidden rounded-2xl sm:rounded-3xl border border-champagne/10 bg-brand-dark p-3.5 sm:p-8 lg:p-10 text-champagne shadow-lg shadow-brand-dark/20"
    >
      {/* ── Statut & Réf & Net Hôte ────────────────────────────────────────── */}
      <div className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-2">
          ...
          {netAmount > 0 && (
            <div className="inline-flex items-center gap-1.5 rounded-full border border-[#4ADE80]/30 bg-emerald-400/10 px-2.5 sm:px-3.5 py-0.5 sm:py-1.5 text-xs font-bold text-emerald-400">
      ...
      <div className="mt-3 flex items-start gap-3 sm:hidden">
        ...
        <div className="min-w-0 flex-1 space-y-1 pt-0.5">
          ...
          <div className="flex items-center gap-1.5 text-xs text-champagne/80">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-champagne/70 truncate pt-0.5">
            <DeliveryIcon className="h-3 w-3 shrink-0 text-champagne/80" />
      ...
      <div className="hidden sm:grid sm:mt-6 gap-6 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-center lg:gap-10">
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-white/5 ring-1 ring-champagne/15 sm:aspect-[16/9] lg:col-start-2 lg:row-start-1 lg:aspect-[4/3]">
          ...
          {vehicule?.type && (
            <span className="absolute right-3 top-3 rounded-full bg-brand-dark/85 px-3 py-1 text-xs font-semibold text-champagne">
              {vehicule.type}
            </span>
          )}
        </div>
        ...
        <div className="min-w-0 space-y-5 lg:col-start-1 lg:row-start-1">
          <div>
            <h2 className="font-display text-4xl leading-[1.05] text-champagne sm:text-5xl">
              {vehicleName}
              {vehicule?.annee ? (
            ...
            <div className="mt-3 inline-flex items-center gap-2 rounded-2xl border border-champagne/15 bg-white/[0.05] px-3.5 py-2 text-xs text-champagne">
              <UserCheck className="h-4 w-4 text-emerald-400" />
              <span className="font-medium text-champagne">Locataire :</span>
              <span className="font-bold text-white">{tenantName}</span>
              {locataire?.kycStatus === 'VERIFIE' && (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full border border-[#4ADE80]/20">
          ...
          <ul className="flex flex-wrap gap-2">
            ...
            {horsDakar && (
              <li className={chipClass}>
                <Navigation className="h-3.5 w-3.5 text-emerald-400" strokeWidth={1.75} aria-hidden="true" />
          ...
          <p className="flex items-start gap-2 text-sm text-champagne/80">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" strokeWidth={1.5} aria-hidden="true" />
