...
/// **ProfileViewModel**
///
/// Gère la logique de l'écran des paramètres de profil utilisateur (Tenant).
///
/// **IMPORTANT:** Ce ViewModel invalide le cache userProfileProvider
/// après chaque modification pour garder tout synchronisé.
class ProfileViewModel extends BaseViewModel<session_service.UserSession?> {
  ProfileViewModel({
  ...
  Future<void> loadProfile() async {
    ...
    if (currentSession == null) {
      state = const ViewState.empty(message: 'Utilisateur non connecté');
      return;
    }
  ...
  Future<void> deleteAvatar() async {
    ...
    await result.fold(
      ...
      (_) async {
        // Enlever l'avatar de la session
  ...
  /// Change l'espace de travail (Mode locataire <=> Espace propriétaire)
  Future<void> switchWorkspace(session_service.UserRole targetRole) async {
    ...
    await result.fold(
      ...
      (authUser) async {
        ...
        // Effet one-shot de succès et retour à l'accueil du rôle ciblé
